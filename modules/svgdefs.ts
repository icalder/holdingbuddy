...
export class Defs {    
    ...
    constructor(svg: SVG.Svg) {        
        ...
        this.stopwatch = this.defineStopwatch(svg);
        this.plane = this.definePlane(svg);
    }

    protected defineStopwatch(svg: SVG.Svg): SVG.G {
        const stopwatch = svg.defs().group();
        stopwatch.circle(10).addClass('stopwatch');
        return stopwatch;
    }
